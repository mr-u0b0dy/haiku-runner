...
4. **`audio_router`** (`audio_router.c`/`.h`) applies `volume_control`'s current gain to accepted frames (`struct audio_frame`, see `audio_frame.h`), then forwards them from the active source to the backend. **`volume_control`** (`volume_control.c`/`.h`) is a single digital gain/mute stage shared by every input, since the MAX98357A has fixed analog gain — its scale matches the Bluetooth VCS `Volume_Setting` field (0 silent, 255 unity), and the BLE adapter's VCP Volume Renderer callback (`vcs_state_cb` in `le_audio_bap_sink.c`) is its only real-world driver today.
