...
Enable it with an overlay config:

```
CONFIG_HR_BACKEND_I2S=y
...
```bash
west build -b nrf5340dk/nrf5340/cpuapp app --sysbuild -- -DEXTRA_CONF_FILE=<path-to-overlay.conf>
west flash
```
...
This only exercises the backend in isolation; it does not touch `source_manager` or any input adapter, and `audio_backend_write()` still ignores real pipeline frames — see [Hardware Porting](/hardware-porting) for wiring the backend into the full pipeline once the hardware path is confirmed good.
...
- `app/boards/nrf5340dk_nrf5340_cpuapp.overlay` already carries the `i2s0` pinctrl/clock config matching the table above (adapted from Zephyr's `i2s/output` sample), so no extra devicetree work is needed to run the self-test tone. `audio_backend_i2s.c` still only drives real hardware when `CONFIG_HR_BACKEND_I2S_TEST_TONE=y`; with it off, the backend remains the original no-op placeholder, and `audio_backend_write()` doesn't yet forward real pipeline PCM to `i2s0` either way — that integration is separate follow-up work.
